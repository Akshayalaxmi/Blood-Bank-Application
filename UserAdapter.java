...
import com.example.ex1.Email.JavaMailApi;
...
public class UserAdapter extends  RecyclerView.Adapter<UserAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        final String idOfheReceiver = user.getId();
                                ...
                                reference.addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        ...
                                        String mMessage = "Hello "+nameOfTheReceiver+","+nameOfSender+
                                                "would like blood donation from you.Here's his/her detilas:\n"
                                                +"Name:"+nameOfSender+"\n"+
                                                "Phone number:"+phone+"\n"+
                                                "Email:"+email+"\n"+
                                                "Blood Group:"+blood+"\n"+
                                        ...
                                        senderRef.child(idOfheReceiver).setValue(true).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if(task.isSuccessful()){
                                                    DatabaseReference receiverRef = FirebaseDatabase.getInstance().getReference("emails")
                                                            .child(idOfheReceiver);
                                                    receiverRef.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(true);

                                                    addNotification(idOfheReceiver,FirebaseAuth.getInstance().getCurrentUser().getUid());
                                                }
                                            }
                                        });
